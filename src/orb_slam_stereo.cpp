...
#include "orbslam3_ros2/image_grabber_stereo.hpp" // Adjust package name if needed
#include <queue>
#include <mutex>
...
void sync_callback(std::shared_ptr<ImageGrabber> igb) {
    while (rclcpp::ok()) {
        ...
        data_cond.wait(lock, [] { return !LeftImgBuf.empty() && !RightImgBuf.empty(); });

        auto msgLeftImg = LeftImgBuf.front();
        auto msgRightImg = RightImgBuf.front();
        LeftImgBuf.pop();
        RightImgBuf.pop();
        lock.unlock();

        igb->processImages(msgLeftImg, msgRightImg); // Pass directly for processing
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // Create SLAM system
    auto SLAM = std::make_shared<ORB_SLAM3::System>(vocab_path, config_path, ORB_SLAM3::System::STEREO, showPangolin);
